...
from app.models import OAuthPayload
...
class CloverOAuthService:
    ...
    async def exchange_code_for_token(self, code: str, merchant_id: str | None = None):
        ...
        payload = OAuthPayload(settings.CLOVER_CLIENT_ID, settings.CLOVER_CLIENT_SECRET, code, settings.CLOVER_REDIRECT_URI)
        ...
        async with httpx.AsyncClient() as client:
            response = await client.post(token_url, json=payload.to_dict(), headers=headers)
